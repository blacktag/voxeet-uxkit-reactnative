...
export const CONSUMER_KEY = 'CHANGE_ME';
export const CONSUMER_SECRET = 'CHANGE_ME';
export const CONFERENCE_ALIAS = 'DEMO';
...
class VoxeetEnvironment {
  ...
  constructor() {
    ...
    this._events.addListener('ParticipantAddedEvent', e =>
      this.emit('ParticipantAddedEvent', e),
    );
    this._events.addListener('ParticipantUpdatedEvent', e =>
      this.emit('ParticipantUpdatedEvent', e),
    );
    this._events.addListener('StreamAddedEvent', e =>
      this.emit('StreamAddedEvent', e),
    );
    this._events.addListener('StreamUpdatedEvent', e =>
      this.emit('StreamUpdatedEvent', e),
    );
    this._events.addListener('StreamRemovedEvent', e =>
      this.emit('StreamRemovedEvent', e),
    );
  }
  ...
  private statusMatching(statuses: ConferenceStatus[]) {
    const matching = Object.keys(this._statuses).find(
      c => !!statuses.find(s => s == this._statuses[c].status),
    );
  ...
  public initialize = async () => {
    await VoxeetSDK.initialize(CONSUMER_KEY, CONSUMER_SECRET, true);
    this._init = true;
  ...
  private onConferenceStatus = (status: ConferenceStatusUpdatedEvent) => {
    ...
    try {
      ...
      if (!status.conferenceId || status.conferenceId.length == 0) {
        //if no conferenceId
